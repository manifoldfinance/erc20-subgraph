...
export function handleTransfer(event: Transfer): void {
  ...
  if (token != null) {
    ...
    let isBurn =
      token.flags.includes('burnable-transfer') && event.params.to.toHex() == GENESIS_ADDRESS;
    let isMint =
      token.flags.includes('mintable-transfer') && event.params.from.toHex() == GENESIS_ADDRESS;
